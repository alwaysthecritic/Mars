...
class MissionRunnerSpec extends FlatSpec with ShouldMatchers {
    ...
    "MissionRunner" should "produce expected output for missions" in {
        ...
        results(0) should be (RobotSituation(1, 1, Direction.East))
        results(1) should be (RobotSituation(3, 3, Direction.North)) // qq lost
        results(2) should be (RobotSituation(2, 3, Direction.South))
    }
}